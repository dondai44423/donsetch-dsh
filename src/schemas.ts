...
export function isValidPrefix(prefix: unknown): boolean {
...
const DROPPED_KEYWORD_MESSAGE: Record<string, string> = {
  maxItems: 'maxItems',
  minItems: 'minItems',
  minLength: 'minLength',
  maxLength: 'maxLength',
...
function droppedConstraintNote(node: Record<string, unknown>): string | null {
  const notes: string[] = []
 ...
 */
function valueSchema(node: unknown, required: boolean): Record<string, unknown> {
  ...
  if (oneOf !== null) {
    ...
    for (const branch of oneOf) {
      ...
      if (Object.keys(converted).length > 0) branches.push(converted)
    }
  ...
  if (raw !== null && typeof raw.type === 'string' && SPEC_TYPES.has(raw.type)) type = raw.type
  ...
  switch (type) {
    case 'object': {
      ...
      if (raw !== null && isPlainObject(raw.properties)) {
        ...
        for (const [name, child] of Object.entries(raw.properties)) {
          ...
          if (Object.keys(converted).length > 0) props[name] = converted
        }
    ...
    case 'array': {
      ...
      if (raw !== null && raw.items !== undefined) {
        ...
        if (Object.keys(converted).length > 0) out.items = converted
        else out.items = { type: 'json' }
      }
      return out
    }
    default: {
      ...
      if (type !== 'json' && raw !== null) {
        if (Array.isArray(raw.enum)) {
          ...
          if (entries.length > 0) out.enum = entries
        }
        if (raw.const !== undefined && enumMatches(raw.const, type)) out.const = raw.const
        if (out.enum !== undefined && out.const !== undefined) delete out.enum
      }
      return out
    }
  }
...
function enumMatches(entry: unknown, type: string): boolean {
  if (type === 'null') return entry === null
  if (entry === null) return false
  if (type === 'integer') return typeof entry === 'number' && Number.isInteger(entry)
  if (type === 'number') return typeof entry === 'number' && Number.isFinite(entry)
 ...
 */
export function specToJsonSchema(spec: Record<string, unknown>): Record<string, unknown> {
  return propertyMapToJsonSchema(spec, 'parameters')
}

function propertyMapToJsonSchema(spec: unknown, path: string): Record<string, unknown> {
  ...
  for (const [name, value] of Object.entries(isPlainObject(spec) ? spec : {})) {
    ...
    const { required: isRequired, ...node } = value as Record<string, unknown>
...
function nodeToJsonSchema(node: unknown, path: string): Record<string, unknown> {
  ...
  switch (type) {
    case 'object': {
      ...
      if (Array.isArray(nested.required)) out.required = nested.required
      return out
    }
    case 'array': {
      out.type = 'array'
    ...
    default: {
      ...
      if (raw.const !== undefined) out.const = raw.const
      return out
    }
  }
}
 ...
 */
export function specViolation(spec: unknown): string | null {
  return propertyMapViolation(spec, 'parameters')
}

function propertyMapViolation(input: unknown, path: string): string | null {
  if (!isPlainObject(input)) return `${path} must be an object of value schemas`
  for (const [name, right] of Object.entries(input)) {
    if (!isPlainObject(right)) return `${path}.${name} must be a value schema object`
...
function valueSchemaViolation(input: unknown, path: string, allowRequired: boolean): string | null {
  if (!isPlainObject(input)) return `${path} must be a value schema object`
  const authorKeys = ['type', 'description', 'title', 'enum', 'const', 'oneOf', 'properties', 'additionalProperties', 'items']
  if (allowRequired) authorKeys.push('required')
  for (const key of Object.keys(input)) {
    if (!authorKeys.includes(key)) return `${path}: unsupported keyword ${JSON.stringify(key)}`
  }
  if (Object.hasOwn(input, 'description') && typeof input.description !== 'string') return `${path}.description must be a string`
  if (Object.hasOwn(input, 'title') && typeof input.title !== 'string') return `${path}.title must be a string`
  ...
  if (hasOneOf && hasType) return `${path} cannot declare both type and oneOf`
  if (hasOneOf) {
    const allowed = allowRequired ? ['oneOf', 'description', 'title', ...(allowRequired ? ['required'] : [])] : ['oneOf', 'description', 'title']
    for (const key of Object.keys(input)) {
      if (!allowed.includes(key)) return `${path}: ${JSON.stringify(key)} cannot accompany oneOf`
    }
    if (!Array.isArray(input.oneOf) || input.oneOf.length < 2) return `${path}.oneOf must be an array of at least two value schemas`
  ...
  if (!hasType) return `${path} must declare a type or oneOf`
  const type = input.type
  if (typeof type !== 'string' || !SPEC_TYPES.has(type)) return `${path}.type must be one of ${[...SPEC_TYPES].join('/')}`
  if (type === 'json') {
    const extras = Object.keys(input).filter((k) => !['type', 'description', 'title'].includes(k) && !(allowRequired && k === 'required'))
    if (extras.length > 0) return `${path}: ${JSON.stringify(extras[0])} cannot accompany type json`
    return null
  }
  if (type === 'object') {
    ...
    if (Object.hasOwn(input, 'properties')) {
      if (!isPlainObject(input.properties)) return `${path}.properties must be an object`
  ...
  if (type === 'array') {
    if (!Object.hasOwn(input, 'items') || !isPlainObject(input.items)) return `${path}.items must be a value schema`
  ...
  if (extras.length > 0) return `${path}: ${JSON.stringify(extras[0])} cannot accompany type ${type}`
  if (Object.hasOwn(input, 'enum')) {
    if (!Array.isArray(input.enum) || input.enum.length === 0) return `${path}.enum must be a non-empty array`
    for (const entry of input.enum) {
      if (!enumMatches(entry, type)) return `${path}.enum value ${JSON.stringify(entry)} does not match type ${type}`
    }
  }
...
export function callKind(toolName: string): 'fetch' | 'search' | 'other' {
  if (toolName.includes('search')) return 'search'
  if (toolName.includes('fetch') || toolName.includes('crawl')) return 'fetch'
  return 'other'
}
...
export function callTitle(toolName: string, args: unknown): string {
  if (args !== null && typeof args === 'object') {
    ...
    if (typeof a.query === 'string' && a.query.length > 0) return `${toolName} \u00B7 "${truncate(a.query, 56)}"`
...
export function firstLine(text: string, max: number): string {
  for (const raw of text.split('\n')) {
    const line = raw
      .replace(/^\s*(#{1,4}|\*\*|###)\s*/g, '')
      .trim()
    if (line.length > 0) return truncate(line, max)
  }
  return ''
}
